...
type IJWT interface {
	GenerateJWT(id int, isValid bool, seconds int) string
	GenerateRefreshToken(id int, isValid bool, seconds int) (string, time.Time)
	GenerateTokenCookie(access, refresh string, expAfter time.Time) *http.Cookie
...
func (j *JWT) GenerateJWT(id int, isValid bool, seconds int) string {
	claims := Claims{
		ID:      id,
		IsValid: isValid,
...
func (j *JWT) GenerateRefreshToken(id int, isValid bool, seconds int) (string, time.Time) {
	...
	secret, err := token.SignedString([]byte(j.secret))
	if err != nil {
		logrus.Errorf("[jwt][id:%d] error generating jwt", id)
		return "", exp
	}
...
func (j *JWT) RefreshAccessToken(refreshToken string) (secret string, expAt time.Time, err error) {
	...
	secret = j.GenerateJWT(claims.ID, claims.IsValid, Minute*10)
